...
> [!IMPORTANT]
> 상태: `HISTORICAL / COMPLETED (v0.1)`. 아래 미체크 step은 당시 실행 형식을 보존한 것이며
...
export function storageContract(name: string, factory: () => ApiKeyStorage): void {
  describe(`ApiKeyStorage contract: ${name}`, () => {
    ...
    it('listByTenant excludes revoked by default', async () => {
      ...
      await storage.insert(fixture({ id: 'b', prefix: 'bbbbbbbbbbbb', revokedAt: new Date() }));
...
import type { ApiKeyContext, ApiKeyRecord, CreateApiKeyInput, CreateApiKeyResult } from './types';
...
export class ApiKeysService {
  ...
  async list(tenantId: string, opts: { includeRevoked?: boolean } = {}): Promise<ApiKeyRecord[]> {
...
describe('ApiKeysGuard', () => {
  ...
  it('enforces @RequireEnvironment', async () => {
    ...
    jest
      .spyOn(reflector, 'getAllAndOverride')
      .mockImplementation((meta: string) => (meta === ENVIRONMENT_METADATA ? 'live' : undefined));
    await expect(guard.canActivate(ctx({ authorization: `Bearer ${key}` }))).rejects.toMatchObject({
...
export class ApiKeysGuard implements CanActivate {
  ...
  async canActivate(exec: ExecutionContext): Promise<boolean> {
    ...
    if (
      requiredScope &&
      !scopeSatisfies(context.scopes, requiredScope.resource, requiredScope.level)
    ) {
